...
if L>8 or L<6: #el if es un condicional que devuelve positivo si la condicion posterior se cumple
    ...
    if not password[1].islower():# el if not hace que si no cumple estos parametros salga un chivato positivo, que activa el condicional y printea la frase. #el islower ve si es minuscula
        Err=Err+("Error en el caràcter 2 ")
        incorrecto=True
    if not password[2].isupper():#el isupper ve si es mayuscula
        Err=Err+("Error en el caràcter 3 ")
        incorrecto=True
    if password[3] not in "*_@":#si no esta en dentro de ese parametro, sale el print
        Err=Err+("Error en el caràcter 4 ")
        incorrecto=True
    if not password[4].islower():
        Err=Err+("Error en el caràcter 5 ")
        incorrecto=True
    if password[5] not in "6789":#si no esta en dentro de ese parametro, sale el print
    ...
    if L>6:
        if password[6] not in"&/#":#si no esta en dentro de ese parametro, sale el print
            ...
            incorrecto=True
        if L>7:
            if not password[7].isnumeric or int(password[7])>5: #el isnumeric ve si es un numero
